...
let usernames2 : string[] = ["Shahid","Dilber","Adnan","Admin","Azam","Ramzan"];
usernames2 = [];
...
function largeShirt (label : string, size : string = "Large ") {
    return size + label;
}
...
function mediumShirt (label : string, size : string = "Medium ") {
    return size + label;
}
...
function anySizeShirt (label : string, size : string) {
    return size + label;
}
...
function my_Album (artistName : string, albumTitle : string, numberofTracks? : number) {
...
// A way to make a flexible list
 function craetObjectWithDynamicKey (key : string, value : string) {
     let dynamicObject = {};
